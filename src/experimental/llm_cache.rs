...
pub struct CostSavingsReport {
    ...
    pub llm_calls: u64,
    /// Estimated cost per LLM call in cents.
    pub cost_per_call_cents: f64,
    /// Total estimated cost without cache.
    pub total_cost_without_cache_cents: f64,
    /// Total estimated cost with cache.
    pub total_cost_with_cache_cents: f64,
...
/// Tracks and reports on the cost savings achieved by the LLM cache.
pub struct CostAnalytics {
    cost_per_call_cents: f64,
    total_queries: std::sync::atomic::AtomicU64,
    cache_hits: std::sync::atomic::AtomicU64,
}

impl CostAnalytics {
    /// Create a new cost analytics tracker.
    ///
    /// `cost_per_call_cents` is the estimated cost of a single LLM API call.
    pub fn new(cost_per_call_cents: f64) -> Self {
        Self {
            cost_per_call_cents,
    ...
    pub fn report(&self) -> CostSavingsReport {
        ...
        CostSavingsReport {
            ...
            cache_hits: hits,
            llm_calls: misses,
            cost_per_call_cents: self.cost_per_call_cents,
            total_cost_without_cache_cents: cost_without,
            total_cost_with_cache_cents: cost_with,
...
mod tests {
    ...
    fn test_cost_analytics() {
        ...
        assert!((report.savings_pct - 70.0).abs() < 0.01);
        assert!((report.total_cost_without_cache_cents - 5.0).abs() < 0.01);
        assert!((report.total_cost_with_cache_cents - 1.5).abs() < 0.01);
        assert!((report.savings_cents - 3.5).abs() < 0.01);
    }

    #[test]
    fn test_cost_analytics_empty() {
        let analytics = CostAnalytics::new(1.0);
        let report = analytics.report();
    ...
    #[test]
    fn test_cost_analytics_reset() {
        let analytics = CostAnalytics::new(1.0);
        analytics.record_hit();
        analytics.record_miss();
        assert_eq!(analytics.report().total_queries, 2);

        analytics.reset();
        assert_eq!(analytics.report().total_queries, 0);
    }
}
