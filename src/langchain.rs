...
impl NeedleVectorStore {
    ...
    pub fn new(config: NeedleVectorStoreConfig) -> Result<Self> {
        let collection_config = CollectionConfig::new(&config.collection_name, config.embedding_dimension)
    ...
    pub fn similarity_search_with_filter(
        ...
        let results = if let Some(f) = filter {
            collection.search_with_filter(query_embedding, k, f)?
        } else {
            collection.search(query_embedding, k)?
        };
    ...
    pub fn max_marginal_relevance_search_with_filter(
        ...
        let candidates = if let Some(f) = filter {
            collection.search_with_filter(query_embedding, fetch_k, f)?
        } else {
            collection.search(query_embedding, fetch_k)?
        };
        ...
        let candidate_embeddings: Vec<(SearchResult, Vec<f32>)> = candidates
            .into_iter()
            .filter_map(|result| {
                collection.get(&result.id).map(|(vec, _)| (result, vec.to_vec()))
            })
            .collect();
    ...
    pub fn update_metadata(&self, id: &str, metadata: Value) -> Result<()> {
        let mut collection = self.collection.write();
        collection.update_metadata(id, Some(metadata))
    }
    ...
    pub fn stats(&self) -> CollectionStats {
        let collection = self.collection.read();
        let needle_stats = collection.stats();
