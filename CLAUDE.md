...
server = ["async", "axum", ...]  # HTTP REST API server
web-ui = ["async", "axum"]      # Web-based admin UI
metrics = ["prometheus"]        # Prometheus metrics
hybrid = ["rust-stemmers"]      # BM25 hybrid search
embeddings = ["ort", "ndarray", "tokenizers"]  # ONNX embeddings (unstable)
embedding-providers = ["async", "async-trait", "reqwest"]  # OpenAI/Cohere/Ollama
experimental = []               # Experimental modules (GPU, cloud control, etc.)
full = ["server", "metrics", "hybrid", "web-ui", "embedding-providers", "experimental"]
